...
Push layers with this label to the top of the layer stack one by one.
...
(function() {
	...
	app.beginUndoGroup('Push layers up by one');

	pushLayersUpByOne(composition, matchingLayers);

	app.endUndoGroup();


	function pushLayersUpByOne(composition, layers) {
		var layer, nextLayer;

		for (var i = 0, il = layers.length; i < il; i ++) {
			layer = layers[i];

			if (layer.index === 1) continue;
